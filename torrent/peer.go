...
// 10 type
const (
	MsgChoke         MsgId = 0 // don't upload, blocking
	MsgUnChoke       MsgId = 1 // can upload
	MsgInterested    MsgId = 2 // can download
	MsgNotInterested MsgId = 3 // don't download, blocking
	MsgHave          MsgId = 4
	MsgBitfield      MsgId = 5
	MsgRequest       MsgId = 6 // pieces, start, length
	MsgPiece         MsgId = 7
	MsgCancel        MsgId = 8
)

// if Id is empty -> keep-alive -> skip
type PeerMsg struct {
	Id      MsgId
	Payload []byte
}
...
func NewConn(peer PeerInfo, infoSHA [SHALEN]byte, peerId [IDLEN]byte) (*PeerConn, error) {
	...
	c := &PeerConn{
		...
		Choked:  true,    // default
		peer:    peer,    // opposite id
		peerId:  peerId,  // client id
		infoSHA: infoSHA, // InfoSHA
	}
