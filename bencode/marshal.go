...
// using reflect to convert BObject into the structure field value
// s interface {} is the structure which is accept the value
// Bencode file char stream -> go structure
...
func unmarshalDict(p reflect.Value, dict map[string]*BObject) error {
	...
	for i, n := 0, v.NumField(); i < n; i++ {
		...
		// take the dictionary key value (string/int/list/dictionary)
		fo := dict[key]
		if fo == nil {
			continue
		}
...
// convert structure into bencode char stream
// go structure -> bencode
